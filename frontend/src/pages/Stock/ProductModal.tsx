...
const ProductModal = ({ open, product, produtos, onClose, onSave } : any) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      setImagePreview(product.image ? `http://localhost:3003${product.image}` : null);
  ...
  useEffect(() => {
    if (product && produtos && produtos.length > 0 && !hasNewImageRef.current) {
      ...
      if (produtoAtualizado && produtoAtualizado.image !== product.image) {
        ...
        setImagePreview(produtoAtualizado.image ? `http://localhost:3003${produtoAtualizado.image}` : null);
      }
    }
  }, [produtos, product]);
  ...
  const handleRemoveImage = () => {
    if (formData.image) {
      ...
      if (formData.currentImage) {
        setImagePreview(`http://localhost:3003${formData.currentImage}`);
      } else {
